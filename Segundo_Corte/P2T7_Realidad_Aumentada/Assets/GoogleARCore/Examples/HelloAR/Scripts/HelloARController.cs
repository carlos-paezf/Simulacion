            ...
            if (foundHit)
            {
                if (!isFox){
                    ...
                    else
                    {
                        ...
                        FoxPrefab = Instantiate(prefab, hit.Pose.position, hit.Pose.rotation);

                        // Compensate for the hitPose rotation facing away from the raycast (i.e.
                        // camera).
                        FoxPrefab.transform.Rotate(0, _prefabRotation, 0, Space.Self);
                    ...
                    FoxPrefab.GetComponent<MoveFox>().StartMove(hit.Pose.position);
                }
            }
        }
